...
# CORE PIPELINE
# ---------------------------------------------------------------------
def clean_combined_data(import_path: str, output_path: str, fx_map: dict = FX) -> None:
    # LOAD DATA
    df = pd.read_csv(import_path, low_memory=False)

    # PRICE HANDLING
    df["price_in_eur"] = df["price"] * df["price_overview.currency"].map(fx_map).fillna(0)
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Clean & enrich combined Steam dataset.")
    p.add_argument("-i", "--input", default=DEFAULT_IMPORT_PATH, help="Path to input CSV")
    p.add_argument("-o", "--output", default=DEFAULT_OUTPUT_PATH, help="Path to output CSV")
    return p.parse_args()


def main() -> None:
    args = parse_args()
    clean_combined_data(args.input, args.output)
