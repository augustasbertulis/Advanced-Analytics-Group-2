...
def minmax_weighted_score(summary_df: pd.DataFrame, weights: dict) -> pd.Series:
    ...
    scaled = MinMaxScaler().fit_transform(summary_df[common_cols])
...
# ------------------------ Main function ------------------------
def run_all_kmeans(import_path=IMPORT_PATH, kpis_csv_path=KPI_CSV_PATH,
                   model_configs=MODEL_CONFIGS, vote_weights=MODEL_VOTE_WEIGHTS):
    ...
    for model_key, cfg in MODEL_CONFIGS.items():
        ...
        top_clusters, summary = best_clusters_by_score(grouped, cluster_col, feats, weights, top_n)
        ...
        # Robustness
        X_scaled = StandardScaler().fit_transform(np.nan_to_num(grouped[feats].to_numpy(), nan=0.0))
        labels = grouped[cluster_col]
        robustness_stats[model_key] = {"silhouette": silhouette_score(X_scaled, labels) if len(set(labels))>1 else None}
        models_data[model_key] = X_scaled

    # ---- Plot elbow curves for all models ----
    ...
    if vote_weights:
        merged["weighted_votes"] = sum(merged[f"is_best_{m}"].astype(float)*vote_weights.get(m,1.0) for m in MODEL_CONFIGS.keys())
